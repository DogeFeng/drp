...
<%!
    public static final String GOODS_LIST_URL = "pages/front/goods/goods_list.action" ;
    private int foot = 1 ;
%>
                    ...
                    <div class="row">
                        <c:forEach items="${allSubtype}" var="subtype">
                            <c:if test="${subtype.wiid == witem.wiid}">
                                <div class="col-md-4"><a href="<%=GOODS_LIST_URL%>?stid=${subtype.stid}">${subtype.title}</a></div>
                            </c:if>
                        </c:forEach>
                    </div>
                </div>
